...
if not os.path.exists(MODELS_DIR):
    raise FileNotFoundError(f"❌ Dossier 'models' introuvable : {MODELS_DIR}")

# Charger le détecteur CNN (sans interface graphique)
cnn_model_path = os.path.join(MODELS_DIR, "mmod_human_face_detector.dat")
if not os.path.exists(cnn_model_path):
    raise FileNotFoundError(f"❌ Fichier introuvable : {cnn_model_path}")
...
def register_child():
    ...
    print("📞 parent_phone_2 reçu :", parent_phone_2)
    ...
    try:
        ...
        # Détecter les visages avec le détecteur CNN (sans libX11)
        gray_image = np.array(Image.fromarray(image_np).convert('L'))
        detections = cnn_face_detector(image_np, 1)  # ← INDENTATION CORRIGÉE
        faces = [d.rect for d in detections]         # ← INDENTATION CORRIGÉE
        ...
        face_embedding = np.array(embedding).tolist()
